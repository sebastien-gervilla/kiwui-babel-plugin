...
import { generateVariableDeclaration } from ".";

const runFirstTest = () => it("generates variable declaration for object pattern", () => {
    const objectPattern = types.objectPattern([
      types.objectProperty(types.identifier("x"), types.identifier("x")),
      types.objectProperty(types.identifier("y"), types.identifier("z")),
    ]);
    ...
    const expected = "let { x: x, y: z } = obj";
    expect(result).toBe(expected);
});
 
const runSecondTest = () => it("generates variable declaration for array pattern", () => {
...
const runThirdTest = () => it("generates variable declaration for identifier", () => {
...
const runFourthTest = () => it("generates variable declaration with init expression", () => {
    const identifier = types.identifier("x");
...
const runFifthTest = () => it("generates empty string for unsupported declaration", () => {
